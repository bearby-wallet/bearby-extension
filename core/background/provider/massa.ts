...
import { signAsync, verifyAsync } from "lib/crypto/ed25519";
...
export class MassaControl {
  ...
  async getAddresses(
    ...addresses: string[]
  ): Promise<JsonRPCResponseNodeStatusAddresses[]> {
    const body = this.provider.buildBody(JsonRPCRequestMethods.GET_ADDRESSES, [
      addresses,
    ]);
    return await this.sendJson(body);
  }
  ...
  async executeReadOnlyCall(
    ...data: SCReadData[]
  ): Promise<ExecuteReadOnlyCallResponse[]> {
  ...
  async getBlocks(...blocks: string[]): Promise<MassaBlock[]> {
    const body = this.provider.buildBody(JsonRPCRequestMethods.GET_BLOCKS, [
      blocks,
    ]);
    return await this.sendJson(body);
  }
  ...
  async getOperations(
    ...
    const body = this.provider.buildBody(JsonRPCRequestMethods.GET_OPERATIONS, [
      operations,
    ]);
    return await this.sendJson(body);
  }
  ...
  async sendTransaction(
    ...
    const body = this.provider.buildBody(
      JsonRPCRequestMethods.SEND_OPERATIONS,
      [data],
    );
    return await this.sendJson(body);
  }
