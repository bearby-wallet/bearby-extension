...
export class TokenControl {
  ...
  async addFT(state: TokenRes) {
    assert(
      state.decimals > 0 || state.decimals < 255,
      INVALID_DECIMALS,
      TokenError,
    );
  ...
  async tokenfetch(addresses: string[]): Promise<TokenRes[]> {
    ...
    const decimalsBody = addresses.map(
      (address) =>
        ({
    ...
    const symbolBody = addresses.map(
      (address) =>
        ({
          max_gas: MIN_GAS_READ,
          target_address: address,
    ...
    const balanceBody = addresses.map(
      (address) =>
        ({
          max_gas: MIN_GAS_READ,
          target_address: address,
          ...
          parameter: Array.from(
            new Args()
              .addString(this.#account.selectedAccount?.base58 || "")
              .serialize(),
          ),
        }) as SCReadData,
    );
    ...
    if (resonses.result) {
      ...
      let balanceRes = results
        .slice(limit * 3, limit * 4)
        .map((bytes) => bytesToU256(bytes));
  ...
  async getBalances(): Promise<Balance[]> {
    ...
    const nativeTokensBody = this.#massa.provider.buildBody(
      JsonRPCRequestMethods.GET_ADDRESSES,
      [addresses],
    );
    const ftTokensData: SCReadData[] = addresses
      .map((addr) =>
        tokensAddresses.map((tokenAddr) => ({
          max_gas: MIN_GAS_READ,
      ...
      .flat();
    const ftTokensBody = this.#massa.provider.buildBody(
      JsonRPCRequestMethods.EXECUTE_READ_ONLY_CALL,
      [ftTokensData],
    );
    const [nativeResonses, ftResonses] = await this.#massa.sendJson(
      nativeTokensBody,
      ftTokensBody,
    );
    ...
    if (nativeResonses.result) {
      for (let index = 0; index < nativeResonses.result.length; index++) {
        ...
        let balanceObject: Balance = {
          [MAS.base58]: {
            ...
            candidate: String(
              Number(resonse.candidate_balance) * 10 ** MASSA_DECIMALS,
            ),
          },
          [ROLL.base58]: {
      ...
      if (nativeResonses.error.message) {
        throw new TokenError(
          `code: ${nativeResonses.error.code}, ${nativeResonses.error.message}`,
        );
      }
